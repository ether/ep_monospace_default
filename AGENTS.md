# Agent Guide — ep_monospace_default

DO NOT USE THIS: Make the font default to Monospace in Etherpad Lite.
...
`ep_monospace_default` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_monospace_default`:
