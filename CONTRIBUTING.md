# Contributing to ep_monospace_default

Thanks for helping improve `ep_monospace_default` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_monospace_default` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_monospace_default from your local clone
pnpm run plugins i --path ../ep_monospace_default

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_monospace_default/` are picked up after a server restart.
...
* Etherpad version, ep_monospace_default version, browser + OS
* Steps to reproduce
